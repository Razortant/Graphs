...
\begin{theorem}[Kruskal 1960]
  \label{theo:Kruskal} \lean{Kruskal} 
...
\begin{proof}
  ...
  For any tree \(T\) with root \(r\), for all \(u,v\in T\), we write \(u<_Tv\) if the unique path from \(r\) to \(v\) passes through \(u\)
  ...
  \(T_0,...,T_{n-1}\) is a bad prefix so there exists \(\left(A_i\right)_{i\in\mathbb{N}}\) a bad sequence with \(T_0,...,T_{n-1}\) as a prefix

  \(A_n\in\mathcal{T}\) so \(\mathcal{T}\) is non-empty
